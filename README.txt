...
---------------------
- Added a dedicated report for licenses that belong to more than one LTG in
  the current IQ data and/or the reconciled output.
- Added alias resolution reporting for source license IDs that reconcile to a
  different IQ license ID.
- Added support for certain `-UNSPECIFIED` source license IDs that need to map
  to a different local IQ license ID in order to preserve existing LTGs.
...
Optional: show the application version

python3 reconcile_ltg.py --version
...
- license_threat_groups_resolved_license_aliases.csv
  Source license IDs that were resolved to a different IQ license ID during
  reconciliation, such as certain `-UNSPECIFIED` source IDs that map to a
  local IQ license ID without that suffix.
...
- license_threat_groups_multiple_groups_report.csv
  Licenses from the source CSV that belong to more than one LTG in the
  current IQ data and/or in the reconciled output, with both views shown
  side-by-side.
...
- If a source license ID ends with `-UNSPECIFIED`, the script first prefers an
  exact match from the IQ export. When that exact ID does not preserve the
  current IQ mapping, the script can fall back to a unique alias derived from
  the stripped ID or a matching `shortDisplayName` in `license.json`.
